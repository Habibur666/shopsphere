...
def _empty_to_none(value):
    """Converts empty-string / missing form fields to NULL for optional columns."""
...
def create_product(data, thumbnail_filename):
    ...
    values = [
        _empty_to_none(data.get("category_id")),
        _empty_to_none(data.get("subcategory_id")),
        _empty_to_none(data.get("brand_id")),
        data.get("name"),
        data.get("short_description") or None,
        data.get("full_description") or None,
        thumbnail_filename,
        _empty_to_none(data.get("price")),
        _empty_to_none(data.get("discount")) or 0,
        _empty_to_none(data.get("offer_price")),
        _empty_to_none(data.get("cost_price")),
        _empty_to_none(data.get("stock")) or 0,
        _empty_to_none(data.get("min_stock")) or 5,
        _empty_to_none(data.get("weight")),
        data.get("dimensions") or None,
        data.get("material") or None,
        data.get("color") or None,
        data.get("size") or None,
        data.get("warranty") or None,
        data.get("return_policy") or None,
        1 if data.get("is_featured") in ("1", "true", True) else 0,
        data.get("status", "draft"),
    ]
    placeholders = ", ".join(["%s"] * len(columns))
...
def update_product(product_id, data, thumbnail_filename=None):
    ...
    numeric_fields = {
        "category_id", "subcategory_id", "brand_id", "price", "discount",
        "offer_price", "cost_price", "stock", "min_stock", "weight",
    }
    updates, params = [], []
    for c in columns:
        if c in data:
